...
async function getUserInfo() {
    ...
    async function onCreateUsers() {
        ...
        let typeDocumentCreate =
            document.getElementById("typeDocumentCreate").value;
        ...
        let passwordConfirmCreate = document.getElementById(
        ...
        if (passwordCreate.length > 100) {
            alert("Password must be less than 100 characters");
            return;
        }
        const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailPattern.test(emailCreate)) {
            alert("Invalid email format");
            return;
        }
        try {
